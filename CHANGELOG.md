...
### Fixed
- `GET /history/orders` didn't return whether a dish was already disliked, so the "Disliked ✓" mark on a History card was lost on every page reload (it only lived in local React state, never in the API response) even though the dislike itself was correctly persisted and still excluded from recommendations. `list_orders` now cross-references the `Dislikes` table.
