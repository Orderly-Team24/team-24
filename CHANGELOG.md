...
### Fixed
- The two backend URLs (`https://team-24.onrender.com`, `https://team-24-1.onrender.com`) were hardcoded separately in 7 frontend page files instead of one shared place, so they'd drifted out of sync in the past (see previous release note). Consolidated into `src/new-frontend/src/config.js`, overridable via `REACT_APP_API_URL`/`REACT_APP_UPLOAD_URL` at build time — needed so the frontend can point at a new deployment (e.g. after transferring hosting) without editing source.
