...
- **`order_history.py` persists to PostgreSQL** (`order_history`/`dislikes` tables, see ADR-002) via a `db: Session` parameter — same `get_db` dependency pattern as `auth.py`/`users.py`. It used to be an in-memory `dict` that restarts wiped; issue #338 was intended to fix this in Sprint 5 and was closed, but **the underlying bug is still reproducible** — order history is not reliably persisting/displaying correctly despite the DB-backed fix. Don't assume #338 fully resolved this; treat it as still open in practice. Likely suspects worth checking first: the same `localStorage.userId` inconsistency described below (order history appearing empty / pooling under the wrong identity is exactly the symptom pattern that bug causes), and whether `get_db` session commits are actually happening on the write path. There's no `/history/_reset` endpoint anymore — it was a dev/test convenience removed once the store was backed by a real database. Tests reset state via `order_history.reset_for_tests(db)` called directly, not over HTTP.
